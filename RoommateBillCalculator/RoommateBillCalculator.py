from CalculatorClasses import Bill, Roommate, PdfReport
## CLI IMPLEMENTATION
...
bill_instance = Bill(amount=bill_amount, period=bill_period)
...
user = Roommate(name=user_name, days_in_unit=user_occupancy)
...
user_roommate = Roommate(name=roommate_name, days_in_unit=roommate_occupancy)
...
bill_report = PdfReport(file_name="Roommate-Bill-for-"+str(bill_instance.period).replace(" ","-")+'.pdf')
